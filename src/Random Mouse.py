...
def decide_direction(right_obstacle, left_obstacle, front_obstacle):
    # Random mouse algorithm: choose randomly between available directions
    available_directions = []

    if front_obstacle > FRONT_OBSTACLE_DISTANCE:
        available_directions.append("forward")

    if right_obstacle > SIDE_OBSTACLE_DISTANCE:
        available_directions.append("right")

    if left_obstacle > SIDE_OBSTACLE_DISTANCE:
        available_directions.append("left")

    # If no available directions, go back
